...
```
Max Buy Price = 当前市场价格 - Total Cushion
```

这意味着：系统只会以**低于当前市场价格一定幅度**的价格提交买单。如果市场价格是 0.52，总安全垫是 0.047，那么系统最高只愿意出 0.473 的价格买入。

> 🎯 **核心思想**：永远不要追高！只在价格足够便宜的时候才出手。

#### 3.2.2 参数影响对照表

为了让你直观感受每个参数对最终决策的影响，我们来看一组数值对比。

**场景假设**：当前价格 = **0.50**，斜率 K = **0.008**，剩余时间 = **60 秒**

| 参数 | 默认值 | 改为 | Total Cushion 变化 | 最大买入价变化 | 影响解读 |
|---|---|---|---|---|---|
| **base_cushion** | 0.02 | **0.05** | 0.047 → **0.077** (+0.03) | 0.453 → **0.423** | 更保守，少花钱买，但也可能错过更多机会 |
| **alpha (α)** | 0.5 | **0.7** | 0.047 → **0.061** (+0.014) | 0.453 → **0.439** | 波动市场更谨慎，buffer 增加 40% |
| **slope_k (\|K\|)** | 0.008 | **0.020** | 0.047 → **0.086** (+0.039) | 0.453 → **0.414** | 大幅波动时大幅降低买入价 |
| **time_remaining** | 60s | **20s** | 0.047 → **0.033** (-0.014) | 0.453 → **0.467** | 时间紧迫时缩小安全垫，提高成交概率 |

**逐行解读**：

1. **base_cushion 从 0.02 调大到 0.05**：
   - 安全垫增加了 0.03（从 4.7 分钱增加到 7.7 分钱）
   - 最大买入价从 0.453 降到 0.423（更便宜才买）
...
4. **剩余时间从 60 秒减少到 20 秒**：
   - 安全垫反而变小了（因为 √20 < √60）
   - **原因**：时间紧迫时，系统愿意适当降低安全要求以提高成交概率
   - **后果**：临近结算时更容易成交，但安全边际也相应减小
...
```
【已知条件】
├─ 当前价格 (Current Price):     0.52
├─ 起始价格 (Start Price):       0.50
...
第五步：计算最大买入价格
  ┌──────────────────────────────────────────┐
  │                                          │
  │ ⑦ Max Buy Price = 当前价格 - Total Cushion │
  │    = 0.52 - 0.047                         │
  │    = 0.473                                │
  │                                          │
  │ ⑧ 取整范围限制:                           │
  │    max(0.01, min(0.99, 0.473)) = 0.47    │
  │                                          │
...
第六步：计算信号置信度
  ┌──────────────────────────────────────────┐
  │                                          │
  │ Confidence = R²×0.5 + 斜率分×0.3 + 差额分×0.2 │
  │                                          │
  │ R² 权重 50%:                              │
  │   0.72 × 0.5 = 0.36                       │
  │                                          │
  │ 斜率权重 30%:                             │
  │   归一化斜率 = min(0.008 × 1000, 1.0)     │
  │             = min(8.0, 1.0)               │
  │             = 0.8（截断到上限）             │
  │   0.8 × 0.3 = 0.24                        │
  │                                          │
  │ 差额权重 20%:                             │
  │   归一化差额 = min(0.02 × 10, 1.0)        │
  │             = min(0.2, 1.0)               │
  │             = 0.2                         │
  │   0.2 × 0.2 = 0.04                        │
  │                                          │
  │ 置信度 = 0.36 + 0.24 + 0.04 = 0.64        │
  │                                          │
  │ ✅ 0.64 ≥ 0.6（最低要求）→ 信号有效        │
  │                                          │
  └──────────────────────────────────────────┘
...
│  Max Buy = 0.52 - 0.077 = 0.443 → 取整 0.44     │
│                                                  │
│  对比默认模式:                                   │
│  • 买入价更低 (0.44 vs 0.47)                    │
│  • 更安全，但成交概率下降                        │
...
│  Max Buy = 0.52 - 0.058 = 0.462 → 取整 0.46     │
│                                                  │
│  对比默认模式:                                   │
│  • buffer 增大 40%，但总价差增幅有限             │
│  • 成交概率略有提升                              │
│  • 适合: 对模型有高度信心的场景                  │
│                                                  │
└──────────────────────────────────────────────────┘
...
| **归一化斜率** | **30%** | 价格变化的强度 | 斜率越大说明趋势越强，信号越值得关注 |
| **归一化价格差额** | **20%** | 实际价格变动的幅度 | 差额越大意味着潜在利润空间越大 |

**归一化处理**：

为了让不同量纲的指标可以在同一公式中比较，需要对斜率和差额进行归一化：

```python
# 斜率归一化（放大 1000 倍后截断到 [0, 1]）
normalized_slope = min(|K| × 1000, 1.0)

# 价格差额归一化（放大 10 倍后截断到 [0, 1]）
normalized_diff = min(|price_diff| × 10, 1.0)
```
...
② 斜率部分:
   原始值: 0.008
   放大:   0.008 × 1000 = 8.0
   截断:   min(8.0, 1.0) = 1.0（达到上限）
   加权:   1.0 × 0.3 = 0.30

③ 差额部分:
   原始值: 0.02
   放大:   0.02 × 10 = 0.2
   截断:   min(0.2, 1.0) = 0.2（未达上限）
   加权:   0.2 × 0.2 = 0.04

④ 总置信度:  0.36 + 0.30 + 0.04 = 0.70
...
| **安全垫公式** | `Total = Base + α × \|K\| × √Time` | 决定了"最多花多少钱买"，是风控的核心 |
| **四重过滤** | 时间、差额、R²、置信度 | 必须全部通过才发出 BUY 信号 |
| **置信度计算** | `R²×0.5 + 斜率×0.3 + 差额×0.2` | 综合评分 ≥ 0.6 才合格 |
| **止损止盈** | 亏损 X% 止损，盈利 Y% 止盈 | 建议比例不低于 2:1 |
| **17 个策略参数** | 分为 5 大类：安全垫/价格/订单/风控/止损 | 新手先用默认值，熟悉后再微调 |
...
> ⚠️ **本章为配置参数字典**，涵盖 `settings.yaml` 中全部 **39 个参数**的完整说明。每个参数均从 [parameter_registry.py](../shared/parameter_registry.py) 提取真实元数据。
...
> **总计：39 个参数** | Strategy: 17 | Connection: 11 | Module: 5 | API: 3 | System: 2 | **敏感参数：4 个**（redis.password + API 三件套）
